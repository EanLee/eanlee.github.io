...
![Gitlab on Docker Hub](../../images/gitlab_ci_same_host/docker_hub_gitlab.png)
...
![GitLab sign-in](../../images/gitlab_ci_same_host/gitlab_sigin_in.png)  
...
![root 預設密碼](../../images/gitlab_ci_same_host/docker_gitlab_initial_root_pwd.png)  

順利登入後，新建立一個名為 Test 的 Repository 後，點選 `Clone` 按鈕後，會發現 `Clone with HTTP` 路徑為 `http://d04070f2213e/[Repository-Name]/test.git`。

![clone path](../../images/gitlab_ci_same_host/gitlab_clone_http_bad.png)

其實，下載的網址內出現的 `d04070f2213e` 字串，其實是 CONTAINER ID。

![docker ps](../../images/gitlab_ci_same_host/docker_ps_only_gitlab.png)  
...
![git clone path](../../images/gitlab_ci_same_host/clone_localhost.png)
...
![Runner setting](../../images/gitlab_ci_same_host/gitlab_registor_runner.png)  

⚠️ 在註冊過程中，會發生會發生 `connect refuse` 的問題。

![runner registory fail: connect refuse](../../images/gitlab_ci_same_host/runnerj_register_failed.png)
...
![network](../../images/gitlab_ci_same_host/docker_network.png)  
...
![network setting of gitlab container](../../images/gitlab_ci_same_host/gitlab_container_networkSetting.png)  
...
![runner registory success](../../images/gitlab_ci_same_host/register_runner_success.png)

成功註冊後，再重新整理 `Setting > CI/CD` 的頁面，會發現原先 Runner 的項目下，出現方才新的 Runner。

![specific runner](../../images/gitlab_ci_same_host/gitlab_specific_Runner.png)  
...
![default gridge network](../../images/gitlab_ci_same_host/default_gridge_network_nouse.png)

而在 `gitlab-network` 的 bridge 網路中，可以找到兩個 Container。

![User-Defined gridge network](../../images/gitlab_ci_same_host/user-define-bridge-network.png)  

再次進行 Runner 的註冊時，`GitLab Url` 就可以使用 DNS 的方式指到 GitLab Container。

![runner registory success](../../images/gitlab_ci_same_host/runner_register_success_2.png)
...
![CD/CD Editor](../../images/gitlab_ci_same_host/gitlab_ci_editor.png)  

⚠️ 此時會發現 CI 卡住。一直在 `Pending`，這是因為 `.gitlab-ci.yml` 內未指定 Runner Tag，GitLab CI 找不到可以用的 Runner。

![CI Pending](../../images/gitlab_ci_same_host/ci_pending.png)
![CI Pending 2](../../images/gitlab_ci_same_host/ci_pending_2.png)

所以需要到 `Settings > CI/CD` 的 Runner，將負責 CI 的 Runner 進行的設定變更，勾選 `Run Untagged Job` 即可。

![runner edit](../../images/gitlab_ci_same_host/gitlab_setting_runner_edit.png)  
![runner edit](../../images/gitlab_ci_same_host/edit_runner_untagged.png)

接著 Redo 之前的 CI Job，還是發生錯誤。

![can't pull git](../../images/gitlab_ci_same_host/cannot_pull_git.png)  

⚠️ Runner 回應 git 的路徑不正確，無法連線。因此，需要額外在 GitLab-runner 的 `etc\gitlab-runner\config.toml` 中，加入參數 `clone-url`。

![add clone url](../../images/gitlab_ci_same_host/clone_url.png)  
...
![Can't reslove host](../../images/gitlab_ci_same_host/gitlab_job_fail_not_resolve_host.png)
...
![add netowrk_mode ](../../images/gitlab_ci_same_host/add_docker_runner_network.png)

再 Redo 之前的 CI Job，終於成功了。

![CI success](../../images/gitlab_ci_same_host/gitlab_ci_job_success.png)
