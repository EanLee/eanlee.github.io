...
![domain name](images/Azure/domain_name.png)

接著，在進行網路相關設定時，Domain Name 有進行規則運算式的驗證，要注意一下。

全部都設定完成後，就是等 Azure 完成部署。

![building](images/Azure/building.png)
...
![圖片20181026_022330](images/Azure/remote_web.png)
...
![ssh_fail](images/Azure/ssh_fail.png)
...
![ssh_success](images/Azure/ssh_success.png)

接下來，就可以從 <http://localhost:8080> 進入 Jenkins。不過第一次登入，一定要進行 Unlock  的動作。

![Jenkins_First](images/Azure/Jenkins_First.png)
...
![Jenkns_working](images/Azure/Jenkns_working.png)
...
1. Mircosoft Azure, [Azure 上的 Jenkins](https://docs.microsoft.com/zh-tw/azure/jenkins/)
2. [雲端運算定義與範疇](https://www.cloudopenlab.org.tw/ccipo_industryDefinition.do)
3. hungys, [認識 Microsoft Azure](https://ithelp.ithome.com.tw/articles/10157344)

# 11. 持續整合 - Jekins on Azure(中)

Eric: 剛剛，我像己經把 Jenkins 在 Azure 上建立起來，也成功的啟動 Jenkins，接下試著用 Jenkins 將 GitHub、BitBucket、Azure DevOps 的 Repository 接起來。

吉米: 串接的過份，不同平台的差異會很大嗎？

Eric: 基本上，越多人便用的平台，Jenkins 通常會出現針對那平台或功能的插件。
...
![bitbucket_create](images/JenkinsonAzure/bitbucket_create.png)

Repository 的設定只要出錯，建置的結果必定失敗。若去看建置失敗的原因，就會看到 git fetch 失敗。

![BitBucket_result](images/JenkinsonAzure/BitBucket_result.png)
...
1. [GitHub Integration: Webhooks](https://support.cloudbees.com/hc/en-us/articles/224543927-GitHub-Integration-Webhooks)
2. [Git Plugin](https://wiki.jenkins.io/display/JENKINS/Git+Plugin)
3. [Configure Jenkins to Run Build Automatically on Code Push (youtube)](https://www.youtube.com/watch?v=ke3f3rcRSc8)
4. [Configuring Jenkins to Run a Build Automatically on Code Push (document)](https://blogs.sap.com/2015/12/15/configuring-jenkins-to-run-a-build-automatically-on-code-push/)
5. [【Jenkins】外掛套件：MSBuild](https://dotblogs.com.tw/echo/2018/03/30/jenkins_plugin_msbuild)
6. alexellis, [3 Steps to MSBuild with Docker](https://blog.alexellis.io/3-steps-to-msbuild-with-docker/)

# 11. 持續整合 - Jekins on Azure(下)

Eric: 前面只是單純針對 Git 版控的 Repository 進行通用設定，但是 GitHub、BitBucket 這些被常用的平台，有好心人士公開專用的插件。
...
![Jenkins_plugin_github](images/JenkinsonAzure/Jenkins_plugin_github.png)
...
![Jenkins_plugin_github_selection](images/JenkinsonAzure/Jenkins_plugin_github_selection.png)
...
1. [GitHub Integration: Webhooks](https://support.cloudbees.com/hc/en-us/articles/224543927-GitHub-Integration-Webhooks)
2. [Bitbucket Plugin](https://wiki.jenkins.io/display/JENKINS/Bitbucket+Plugin)
3. [GitHub Plugin](https://wiki.jenkins.io/display/JENKINS/GitHub+Plugin)
...
![Index](images/azureCI/Index.png)

選取 Pipelines 的 Builds 後，會看到工作清單。但因為目前沒有任何資料，所以直接進入**建立 pipeline** 的容量。

![Build_list](images/azureCI/Build_list.png)

選擇 Azure Repository  後，如果該 Repository 內有資料，會列出 Repository 清單。

![Azure pipeline select](images/azureCI/Azure_pipeline_select_repos.png)

如果選取的 Repository 內己經有資料，Azure Pipeline 會自動分析，並建立 yml  檔。

![圖片20181028_225151](images/azureCI/圖片20181028_225151.png)
...
![Azure_pipelines_result](images/azureCI/Azure_pipelines_result.png)
...
1. [Azure DevOps Features Timeline](https://docs.microsoft.com/en-us/azure/devops/release-notes/index)
2. [YAML schema reference](https://docs.microsoft.com/en-us/azure/devops/pipelines/yaml-schema?view=vsts&tabs=schema)
