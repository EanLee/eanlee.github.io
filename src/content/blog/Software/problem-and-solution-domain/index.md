...
lastmod: 2026-03-14T02:27:18+08:00
keywords:
  - 問題領域
  - DDD 基礎
  - 需求分析
...
epic: software
---
我們經常遇到一個情況：業務單位開出的需求，到了開發團隊手上做出來的系統，卻完全不是使用者想要的。問題到底出在哪？很多時候是因為我們沒有區分清楚「問題」與「解法」。

當開始接觸 Domain-Driven Design (DDD) 時，一定會接觸到 `Problem Domain`、`Solution Domain` 、`Context`、`Problem Space` 與 `Solution Space` 這些名詞。

而 `Problem Domain` 與 `Solution Domain` 的概念，用在釐清問題的切入面，現有資訊的評估，都可以帶來許多的幫助。
...
所以為了有效溝通，我們必須存在於相同的領域中，對話才能產生共識。

其實這背後想表達的是：我們在溝通時，必須具備相同的知識背景，並且確保雙方處於同一個情境下進行對話。

因此，軟體設計中有一個非常關鍵的詞彙——Context。這個詞有許多中文翻譯，最常見的是「上下文」或「語境」，但個人比較偏好使用「脈絡」。

我們今天在討論一件事情或與人溝通時，回答和反應往往取決於先前的背景資訊。如果無法知道明確的前置條件或背景知識，就很容易發生「雞同鴨講」的情況，也就是所謂的頻道不在同一條線上。
...
當問題領域越是明確，就更容易鎖定真正要處理的問題點在哪裡。越是理解問題所需要的領域知識，就能更有效的去設計與建構出符合用戶與利害關係人的需求，或是排除系統現有的問題
...
當我們已經明確鎖定了問題領域後，接下來就是要找出對應的解決方案。

這些解決方案通常包含了特定的技術、工具、方法，甚至是系統架構的調整設計。

同時，我們必須考慮如何提出一個能符合用戶需求、技術環境、成本與時間等先天限制的解決方案。
...
面臨的問題，可能是原本問題的延伸，所以可能會需要調整原有的解決方案，或是配合其他現有的解決方案，共同處理掉這個問題。

---

> 💡 **互動時間**
> 釐清了空間與領域的概念後，想了解更多 DDD 的落地實踐嗎？你在專案中有遇過把「解法」當成「需求」的溝通災難嗎？歡迎在下方留言分享你的實戰故事！
