...
標題提到了圖表分析，在這裡舉一個常見的 K6 結合 Grafana 面板的報表範例：

![圖：Grafana k6 測試報表範例，呈現 VUs、RPS、Response Time 與 Error Rate 關聯。](./images/grafana_k6_dashboard_mock.png)

從這種類型的視圖中，我們在報告裡應該如何解讀呢？

1. **觀察負載與響應時間的關係 (VUs vs Response Time)**
   在此範例圖中，併發使用者 (VUs) 從 0 穩定爬升到 2,500。我們可以看到當 VUs 達到 2,500 的高原期時，Response Time P95 依然穩定維持在約 165ms 左右（如圖表中間的 P95 黃線）。這代表系統在 2,500 VUs 的負載下，依然能提供穩定的效能回饋，沒有出現效能劇降的情況。
2. **監控 Error Rate 的異常波動**
   圖表顯示目前的 Error Rate 非常低（約 0.12%），且 HTTP 響應代碼大多為 2xx (Success)。但在 Errors 圖表中若出現突發尖峰，我們就可以精確鎖定是在哪一個 VUs 水位時系統開始出現不穩定（例如 API 逾時、DB Connection 耗盡或 5xx 錯誤）。
3. **對齊 CPU / Memory 指標**
   有了上述的效能表現紀錄，我們需要在報告中進一步串聯對應時段的主機資源。例如此時若主機 CPU 已達到 90%，則可判斷系統已接近擴展（Scale-out）的臨界點，即使響應時間尚未劣化。
