...
哪知好景不常，B 公司推出殺手級的商品，大部份旳消費者轉向購買 B 公司的商品。
A 公司只能將緊急中止新代工廠的訂單，待原本代工廠完成原本的訂單後，停止生產。而已經生產的商品，持續放在倉庫中，等待消費者來購買。
```
...
- 速率不均
  ...
  雖然，A 公司預估預產商品錯誤有誤，讓商品出現售完的情況。爾後，因為滯銷的情況，讓後面生產出來的商品只能存放於倉庫，等待慢慢銷售。
...
在 `.NET Core` 之中，已經針對生產者消費者模型，設計 [IProducerConsumerCollection](https://docs.microsoft.com/zh-tw/dotnet/api/system.collections.concurrent.iproducerconsumercollection-1) 介面。而 `BlockCollection`、`ConcurrentQueue` 均基於 `IProducerConsumerCollection` 的實作，提供安全執行緒集合適用的封鎖和界限容量，
...
在以前，原子被認為是最小的粒子，無法在再分割。雖然現在已經推翻這個概念，但還是習慣使用這個名詞。
...
💬 **參與討論**
非同步處理與 Queue 是每個系統長大的必經之路。你在實作 Message Queue 或 Producer-Consumer 模型時，遇過最難解的併發問題是什麼？留言分享你的血淚史吧！
