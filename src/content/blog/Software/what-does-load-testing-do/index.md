...
description: 釐清效能測試 (Performance)、負載測試 (Load) 與壓力測試 (Stress) 在工程定位上的核心差異。從實務視角梳理不同測試情境的壓測目標與觀測指標，探討如何客觀摸清系統吞吐極限與資源枯竭點。
...
為了幫助大家快速釐清，我整理了這三種測試的核心差異比較表：

| 測試種類 | 主要目的 | 負載強度 | 關注指標重點 | 應用場景 |
| :--- | :--- | :--- | :--- | :--- |
| **效能測試 (Performance)** | 找出系統瓶頸並優化 | 一般日常的穩定負載 | Response Time (響應時間) | 例行性檢視系統是否有拖慢的 SQL 或外部 API |
| **負載測試 (Load)** | 驗證系統是否符合規格 | 高於或等於預期的最大負載 | 處理正確性、資源使用率 | 上線前大拜拜，確認 1000 人同時結帳資料不會錯 |
| **壓力測試 (Stress)** | 找出單機或系統癱瘓的極限 | 持續極端增加直到崩潰 | 崩潰點、Auto Scale 觸發點 | 搶購活動前，評估需要加開多少台機器 |

隨著你的問題與目的不同，測試所規劃的系統架構、測試案例、準備假資料、作法等等，都會有所不同，無法以一概全。
...
### 目的三：找出目前系統運行的瓶頸，以進行後續改善與優化 (效能測試 Performance Testing)
...
> 💡 **互動時間**
> 本文帶你釐清了這三種壓測的核心目的。測試完之後怎麼向老闆或客戶報告呢？請見壓測系列下集：[如何撰寫專業的效能測試報告](../talking-testing-report/index.md)！
> 你在公司都習慣用 k6 還是 JMeter？歡迎在留言區分享你的愛用工具！

## 延伸閱讀

1. [Introduction (k6.io)](https://k6.io/docs/test-types/introduction/)
2. [淺談負載測試/壓力測試/效能測試報告](../talking-testing-report/index.md)
