---
title: 高併發架構導論：系統負載、分流策略與限制理論 (Theory of Constraints) 的實踐
date: 2022-07-26T17:35:47+08:00
lastmod: 2023-12-05T10:26:51+08:00
description: 面對突發流量，系統的真正瓶頸往往在想不到的環節。本文梳理系統負載的本質，並將限制理論 (Theory of Constraints) 應用在後端架構，分享動態分流、背壓與排隊管制的實戰經驗，探討如何守住高吞吐效能防線。
keywords:
  - 系統架構設計
  - 高併發
  - 分流策略
...
但進行壓測時發現，當首頁訪問人數提升到 300 QPS(Query Per Second)時，發現首頁請求等待的時間變長，出現大量的錯誤，直接影響到站台的交易功能。

### 系統負載

![系統與使用率](./images/origin.png)

此時，嘗試重現發現異常的情境。（關於不同種類的壓測目的與報告解讀，可參考我這篇：[效能測試全攻略：從負載與壓力測試策略到讀懂效能報告與架構決策](../performance-load-stress-testing-and-reporting/index.md)）

使用壓測工具，模擬同一時間 300 人同時檢視首頁的同時，中間還穿插交易的請求。發現大量請求湧入時，會造成所有主機的 CPU/Memory 資源使用率滿載。進一步造成大量連線逾時，造成交易失敗。**以業務與收益層面而言，這種情況一定不允許發生的。**
...
只好再進一步的分析最大量的請求內容，評估有無辦法減少首頁的請求對 API 主機或 DB 主機的負擔呢？當觀察這些 API 的行為與回應資料，發現它們資料內容大多重覆。
...
1. **要改變什麼？(What to Change？)** - 找出 DB 過載的瓶頸點。
2. **要改變成什麼？(What to Change to？)** - 建立新的資料分流架構，引入 Cache 來降低 DB 的讀取負擔。
3. **如何改變？(How to Cause the Change？)** - 實作 Cache 讀寫機制，並配合 80/20 法則針對高頻存取的首頁資料先行緩存。

套用上述的電商案例，正是一次完美的高德拉特五步聚焦法實作：

- 我們發現了 DB 與 API 等候是瓶頸 (**步驟一**)。
- 我們決定不能無限制對 DB 發出查詢 (**步驟二：利用瓶頸**)。
- 我們讓非交易的查詢改走 Cache (**步驟三：配合瓶頸調整一切**)。
- 我們成功把 DB 使用率降下來，解除了系統癱瘓風險 (**步驟四：鬆綁瓶頸**)。
- 若未來流量上萬，我們可能又會發現 Cache 變成新的瓶頸，那就是 (**步驟五：重回步驟一**) 的時候了。
...
> 💡 **互動時間**
> 架構演進往往是被逼出來的，你在遇到突發流量暴增時，做過什麼最有效的「分流」或「限流」決策呢？歡迎在下方留言分享你在高併發戰場上的求生指南！
