...
lastmod: 2026-03-14T02:29:24+08:00
...
epic: software
---
大家都知道網址前面有「鎖頭」才代表安全，但 HTTPS 到底是如何幫我們加密的？常聽到的 SSL 跟 TLS 是一樣的東西嗎？這篇文章我們就由淺入深，從網路的基礎模型開始說起。
...
HTTP 本身單純 Request/Response 一來一往的進行通訊。
...
SSL/TLS 的活動位置，位於 OSI 的 Session Layer，針對傳輸資料內容，進行加密與身份驗證，達到保護個人或機敏性資料的目的。
...
數位簽章 (Digital Signature) 就是資料的雜湊值 (hash) 經由簽章者的私鑰簽名後的結果。
...
若想要進一步了解數位簽章與認證的機制，這又是個專門的主題，這邊就不多加描述，有興趣可參考相關資料。

採用對稱金鑰，對資料進行加解密。這組金鑰在 SSL/TLS 建立的過程中，就已經決定好雙方共同使用於加解密的金鑰。

可能會好奇為何不使用非對稱加密演算法，更進一步確保資料的安全？最大因素在於非對稱加密的速度慢。

因此，現代的 HTTPS 傳輸通常是**混合使用**：先用嚴謹的「非對稱加密」來交換「對稱加密的鑰匙」，接著後續大量的網頁資料傳輸，全部改用這把「對稱加密的鑰匙」來進行，以確保傳輸速度。這樣就完美兼顧了安全性與效能。

## 簡單總結

簡單回顧一下 HTTPS (SSL/TLS) 的運作核心：它透過數位憑證來確認伺服器身分（防偽造），接著透過三次握手與複雜的密碼學機制來交換加密金鑰，最後再用該金鑰保護我們傳送的每一筆資料（防竊聽與竄改）。

---

> 💡 **互動時間**
> 了解了憑證與加密的底層原理，實戰中該如何為自己的網站申請憑證呢？
> 看懂了憑證與加密的底層原理，實戰中該如何申請與部署呢？
> 歡迎參考我的其它 IIS/Nginx 實務教學，也歡迎在下方留言分享你曾經踩過的憑證過期大坑！
